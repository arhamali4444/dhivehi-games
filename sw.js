...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;const u=new URL(r.url);
 ...
 e.respondWith(fetch(r).then(res=>{if(res&&res.ok){const cp=res.clone();caches.open(CACHE).then(c=>c.put(r.mode==='navigate'?new Request(u.pathname):r,cp));}return res;})
  .catch(()=>caches.match(r.mode==='navigate'?u.pathname:r).then(m=>m||caches.match(r.mode==='navigate'&&u.pathname.startsWith('/digu')?'/digu/':r.mode==='navigate'&&u.pathname.startsWith('/dhogu')?'/dhogu/':'/'))));});
